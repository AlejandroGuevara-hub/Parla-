...
**Cómo se corrigió:** La imagen dejó de ser un `<img>` absoluto con `transform` (que al desplazarse/expandirse se salía del marco del hero) y pasó a ser un **fondo CSS de la sección** (`.hero__bg` como `background-image`). Un background siempre se recorta dentro de su elemento por definición, así que nunca desborda ni deja ver el fondo del `body`. El parallax ahora mueve `background-position` (contenido dentro de la sección) en lugar de `transform`, con avance acotado a `0–24px`.

**Cómo evitarlo:** Para una imagen de fondo que nunca desborde, usar `background-image` del contenedor (se recorta solo) en vez de un `<img>` que se transforme con `transform`. Si se quiere parallax, mover `background-position` (siempre dentro) y no `transform`. En parallax derivar el avance de la posición del contenedor en el viewport (`getBoundingClientRect()`), no de `scrollY - inicio`.
